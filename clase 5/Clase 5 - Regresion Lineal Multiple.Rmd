...
```{r}
library(GGally)
ggpairs(df[,c(2,4,5,6,7,12)],  mapping = aes(color = df$trabajo_de))
```
...
# Resumen del modelo
resumen_meg <- summary(modelo_exp_gc)
resumen_meg
```

#### Significado de los coeficientes estimados

* El valor de la ordenada al origen (51.668) es el valor de salario neto esperado para alguien sin experiencia laboral (0 años de experiencia) y sin gente a cargo, por lo que no tendría sentido. 
...
resumen_mes <- summary(modelo_exp_sex)
resumen_mes
```
...
resumen_meed <- summary(modelo_exp_edu)
resumen_meed
```
...
resumen_meg$coefficients
confint(modelo_exp_gc) # calculamos intervalo de confianza
```
...
* Además del resultado del test, podemos apreciar que el intervalo de confianza del 95% de la variable años de experiencia no contiene al 0, mientras el IC de la variable gente_a_cargo sí. 

```{r}
# Modelo Años de experiencia y Género
resumen_mes
confint(modelo_exp_sex) # calculamos intervalo de confianza
```
...
* Además del resultado del test, podemos corroborar que los intervalos de confianza del 95% para los coeficientes estimados no contienen al 0.  

**¿Cómo se interpreta la significatividad de las variables dummies?**

* En el caso de la variable dicotómica me_identifico, este test permite chequear si los valores medios del sueldo neto son los mismos para las mujeres respecto de los hombres (categoría basal).
...
resumen_meed
confint(modelo_exp_edu) # calculamos intervalo de confianza
```
...
* Si queremos evaluar a la variable nivel_edu_alcanzado en su conjunto, debemos recurrir a un test F, cuando además en el modelo aparece otra variable como en este caso los años de experiencia. 
...
```{r}
# Modelo Años de experiencia y Género
anova(modelo_exp_sex)
# Modelo Años de experiencia y Nivel Educativo Alcanzado
anova(modelo_exp_edu)
```

Ambas tablas de ANOVA muestran que, según los resultados del test F, en el primer modelo la variable género (me_identifico) en su conjunto es estadísticamente significativa para explicar al sueldo neto; mientras que en el segundo modelo el nivel educativo en su conjunto también (p-valores < 0.05).
...
```{r}
TukeyHSD(aov(modelo_exp_edu),"nivel_edu_alcanzado")
```
...
```
Tienen alta correlación. Veamos qué ocurre con los coeficientes de estas dos variables al armar un modelo con todas las variables que vimos antes juntas, incluidas estas dos que se encuentran altamente correlacionadas, en comparación a un modelo con menos variables. 
...
```{r}
summary(lm(salario_neto ~ edad + nivel_edu_alcanzado + gente_a_cargo + me_identifico, data = df2))
```
