...
* H1: no todos los $β_k$ (k = 1, 2,..., p−1) son iguales a 0
...
```{r}
modelo_5 <- lm(salario_neto ~ anos_de_experiencia + edad + gente_a_cargo + me_identifico, data = df2)
tidy(modelo_5)
```
...
```{r}
modelo_4 <- lm(salario_neto ~ anos_de_experiencia + gente_a_cargo + me_identifico, data = df2)
tidy(modelo_4)
```

¿Qué diferencias encuentran con los coeficientes de edad y años de experiencia en los 3 modelos?

* Los coeficientes de regresión estimados se modifican sustancialmente cuando agregamos o quitamos variables del modelo. En el modelo_5 vs exp_edad el beta estimado de los años de experiencia cambia de 974 a 901 y la edad de -80 a 48. 
* Los errores estándares de los estimadores de los coeficientes aumentan espúreamente cuando se incluyen covariables muy correlacionadas. Se infla la varianza estimada de los estimadores. En nuestro caso: el error estándar de la variable años de experiencia en el modelo_5 donde está incluida la edad es de alrededor de 557, mientras que en el modelo_4 que se excluye dicha variable es de 287. 
...
¿Cómo es la correlación entre las variables que queremos analizar?

```{r}
cor(df2$anos_de_experiencia, df2$anos_en_la_empresa_actual)
```
Están correlacionadas. Veamos cómo afecta la interpretación cuando usamos esas variables para construir un modelo. 
...
```
¿Qué pasa con los coeficientes estimados de la variables? 

Anos_de_experiencia resulta significativa y anos_en_la_empresa_actual no. 

¿Cómo se interpretan estos coeficientes?

xxxxx

¿Qué pasa si creamos un nuevo modelo separando los años de experiencia entre experiencia previa y acutal? 
...
La experiencia previa resulta significativa y los anos_en_la_empresa_actual no. 
...
```{r}
tidy(lm(salario_neto ~  me_identifico + anos_de_experiencia + me_identifico*anos_de_experiencia, data = df2))
```

```{r}
tidy(lm(salario_neto ~  perfil + edad, data = df2))
```