...
* En el modelo de experiencia y edad, los p-valores de los coeficientes estimados superan el umbral de 0.10. Es decir, ninguno de los dos son significativos cuando ambas variables están en el modelo
...
¿Cuál es la interpretación correcta del parámetro correcta del parámetro estimado $\beta_2$?
...
* $\hat{\beta_2}=-431$ indica que por cada año adicional en la empresa actual el salario neto esperado aumenta en 637 pesos dados los años de experiencia previa
...
```{r}
tidy(lm(salario_neto ~  perfil + edad + me_identifico, data = df2))
```