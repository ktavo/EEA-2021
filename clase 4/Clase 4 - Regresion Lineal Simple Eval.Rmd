...
Tiene dos parámetros muy útiles:
  ...
  * `conf.level`: nivel de confianza para calcular los intervalos. Este es valor es $1 - \alpha$ 
...
Al aumentar el nivel de confianza los intervalos se volvieron más anchos. De todas formas, siguen sin contener al cero.
...
En este modelo, el p-valor asociado al test sobre $\beta_1$ es igual a 0.067. Si nuestro nivel de significancia es 0.05 no rechazamos la hipótesis nula.

Esto también se puede observar en el intervalo de confianza asociado ya que el mismo contiene el cero.
...
* p-value: es el p-valos de dicho test (se encuenta al lado del F-statistic)
...
Observamos que el modelo que utiliza los años de experiencia como variable explicativa tiene el mayor R-cuadrado: $R^2=0.1698$ y por lo tanto es el mejor modelo entre los que probamos para **explicar** el salario neto del grupo de personas que estamos analizando.
...
1) **Residuos** vs **valores predichos**:  Parece existir cierta estructura en los datos: una leve curvatura y varianza que depende de la covariable

2) **Normal QQ plot**: los extremos no se ajustan a la distribución teórica

4) **Residual** vs **leverage**: Existen dos puntos con un leverage bastante alto, el gráfico destaca la observación 82.

```{r}
encuesta_sueldos[82,]
```

**Diagnóstico**: el modelo no cumple con los supuestos del modelo lineal. Parecen existir dos problemas: la existencia de heterocedasticidad (varianza no constante) y la presencia de observaciones de alto leverage
...
encuesta_aumentada = augment_columns(x=modelo_edad, data=encuesta_sueldos)
