...
* [__gplot2__](https://ggplot2.tidyverse.org/) para realizar gráficos
...
El paquete tidyr esta pensado para "emprolijar los datos". Tiene un conjunto de funciones para implementar las nociones de Tidy Data desarrolladas por Hadley Wickham en su [paper](http://www.jstatsoft.org/v59/i10/paper).
...
La funcionalidad es similar entre estas versiones es similar, aunque __pivot_longer__ y __pivot_wider__ tienen algunos cambios útiles,   pero __gather__ y __spread__ ya no cuentan con mantenimiento y los autores de la librería recomiendan enfáticamente el uso de las nuevas versiones

![Fuente: http://www.gis-blog.com/data-management-with-r-tidyr-part-1/](spreadVSgather.png)

En la siguiente tabla cada observación es una planta con la medición del largo y ancho de su sépalo y pétalo y el tipo de especie al cual pertenece
...
```{r}
iris_vertical <- iris %>% 
                    pivot_longer(cols=c(Sepal.Length, Sepal.Width, Petal.Length,Petal.Width), # Columnas a pivotear al formato "largo
                                 names_to = "tipo_medicion", # nombre de la nueva columna sobre la que pivoteo el dataframe
                                 values_to = "valor" # nombre de la nueva columna de valores
                                )
iris_vertical
```

### Pivot_wider (ex Spred)

Podemos deshacer el __pivot_longer__ con un __pivot_wider__

```{r}
iris_horizontal <- iris_vertical %>%
                      pivot_wider(names_from = tipo_medicion, # nombre de la variable a expandir
                                  values_from = valor) # nombre de la columna con valores
iris_horizontal
```
...
## [__Stringr__](https://stringr.tidyverse.org/)
