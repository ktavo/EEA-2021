...
1) Crear un dataframe con el dataset de R: state.x77 y contestar las siguientes preguntas:
...
2) Crear un dataset **df_indice** solo con las columnas de porcentaje de analfabetismo sobre población  (*Illiteracy*) y tasa de homicidios por 100.000 habitantes (*Murder*). 
...
2) Crear una función _acumulado_ que calcule el valor acumulado (suma) de una variable numérica a designar X en un dataset también a designar df. Es decir, que brinde el valor resultante de acumulado(df, X). 
...
3) Modificar la función anterior ( _acumulado2_) para que devuelva un vector que contenga la frecuencia poblacional (el acumulado calculado previamente) y la muestral (número de filas del dataset). 

a) Utilizar la función modificada para calcular la frecuencias frecuencias muestrales y poblacionales por Sexo. 
