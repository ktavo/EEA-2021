...
Basado en la correlación observada entre las variables surface_total y surface_covered , y también en la observada entre las variables bathrooms y rooms, genero un modelo más reducido:

Primero evaluamos la correlación entre las variables mencionadas:
```{r}
cor(aptosTrain_pricem2$surface_total, aptosTrain_pricem2$surface_covered)
cor(aptosTrain_pricem2$bathrooms, aptosTrain_pricem2$rooms)
```

Ahora generamos el modelo a 

```{r}
modelo_propiedades_short <- lm(price ~ surface_covered + rooms + property_type + pricem2, data = aptosTrain_pricem2)
# Resumen del modelo
tidy_meg_barrios_short <- tidy(modelo_propiedades_short, conf.int = TRUE)
tidy_meg_barrios_short
```
Todas las variables para este modelo resultan altamente significativas.

Ahora generamos otro modelo donde añadimos una variable para definir si la propiedad cuenta con espacio al aire libre o no:

```{r}
aptosTrain_pricem2$tiene_sup_descubierta <- ifelse(aptosTrain_pricem2$sup_descubierta > 0, TRUE, FALSE)

modelo_propiedades_barrios_has_descubierta <- lm(price ~ surface_covered + bathrooms + rooms + tiene_sup_descubierta + property_type + pricem2 + barrios, data = aptosTrain_pricem2)
# Resumen del modelo
tidy_meg_barrios_has_descubierta <- tidy(modelo_propiedades_barrios_has_descubierta, conf.int = TRUE)
...
De los modelos generados previamente tomaría 
...
*modelo con la variable barrio
Es el de la mejor R2-Ajustada entre los modelos.
*modelo con la variable sup_descubierta
Es el segundo modelo con mejor R2-Ajustada, además, es una variable interesante la de superficie descubierta, peusto que a nivel de negocio permite clasificar las propiedades que tienen ese plus, a nivel de departamento y PH que es el balcón, y a nivelde casa las que poseen patios.
